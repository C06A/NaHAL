...
| `nahal-app-ios-simulator-<version>.zip` (unsigned simulator build), or Xcode over `NahalUI.framework`; `MainViewController()` from the `nahal-ui` klib to embed it
...
The released asset is that same unsigned *simulator* build — install it with
...
| Unsigned iOS *simulator* build of `NaHAL.app`
