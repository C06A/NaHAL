...
val writeNahalConfig by tasks.registering {
    ...
    val apiKey = providers.gradleProperty("apiKey").getOrElse("")
    doLast {
        nahalConfig.get().asFile.writeText(
            """
            {
              "com.helpchoice.nahal.plugin.apikey.ApiKeyPlugin": { "apiKey": "$apiKey" }
            }
            """.trimIndent()
        )
    }
...
tasks.withType<JavaExec>().configureEach {
    if (name == "jvmRun") {
        group = "run"
        description = "Runs the NaHAL desktop UI with the api-key plugin (pass the key with -PapiKey=...)."
