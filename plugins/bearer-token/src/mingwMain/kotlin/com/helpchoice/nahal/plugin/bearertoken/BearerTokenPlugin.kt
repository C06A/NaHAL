...
/**
 * Windows (MinGW) — HALDiSh plugin that injects `Authorization: Bearer <token>`.
 *
 * **Per-platform authoring note:** this file is intentionally self-contained (no commonMain
 * implementation). Copy it as a starting point for a Windows plugin that needs platform-specific
 * logic, e.g. reading a token from the Windows Credential Manager via WinAPI.
 *
 ...
 * ```yaml
 * BearerTokenPlugin:
 *   providerClass: com.example.WinCredentialProvider
 * ```
 * Register the provider before the first HTTP call:
 * ```kotlin
 * BearerTokenProviderRegistry.register(
 *     "com.example.WinCredentialProvider",
 *     WinCredentialProvider()
 * )
 * ```
 *
 ...
 * ```
 * Or set `HALDISH_PLUGIN_PATH` to point to a DLL exporting `haldish_plugin_pre_request`.
 *
