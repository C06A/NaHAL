...
/**
 * Apple (iOS / macOS) — HALDiSh plugin that injects `Authorization: Bearer <token>`.
 *
 * **Per-platform authoring note:** this file is intentionally self-contained (no commonMain
 * implementation). Copy it as a starting point for an Apple plugin that needs platform-specific
 * logic, e.g. reading a token from the iOS/macOS Keychain.
 *
 ...
 * ```yaml
 * BearerTokenPlugin:
 *   providerClass: com.example.KeychainTokenProvider
 * ```
 * Register the provider before any HTTP call (e.g. in `AppDelegate`):
 * ```swift
 * BearerTokenProviderRegistry.shared.register(
 *     fqn: "com.example.KeychainTokenProvider",
 ...
 * ## Programmatic usage
 * ```swift
 * HaldishPluginRegistry.shared.setPlugin(BearerTokenPlugin(staticToken: myToken))
 * ```
 *
