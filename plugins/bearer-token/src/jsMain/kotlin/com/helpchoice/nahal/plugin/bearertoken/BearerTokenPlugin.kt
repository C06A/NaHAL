...
/**
 * JS — HALDiSh plugin that injects `Authorization: Bearer <token>` into every request.
 *
 * **Per-platform authoring note:** this file is intentionally self-contained (no commonMain
 * implementation). Copy it as a starting point for a JS plugin that needs platform-specific
 * logic (e.g. reading a token from a browser cookie, `localStorage`, or an OAuth flow).
 *
