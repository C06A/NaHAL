...
val writeNahalConfig by tasks.registering {
    ...
    val token = providers.gradleProperty("token").getOrElse("")
    doLast {
        nahalConfig.get().asFile.writeText(
            """
            {
              "com.helpchoice.nahal.plugin.bearertoken.BearerTokenPlugin": { "token": "$token" }
            }
            """.trimIndent()
        )
    }
...
tasks.withType<JavaExec>().configureEach {
    if (name == "jvmRun") {
        group = "run"
        description = "Runs the NaHAL desktop UI with the bearer-token plugin (pass the token with -Ptoken=...)."
...
mavenPublishing {
    ...
    coordinates(
        ...
        artifactId = "haldish-plugin-bearer-token",
        version    = project.version.toString(),
    )

    pom {
        name        = "HALDiSh Plugin — Bearer Token"
        description = "HALDiSh plugin that injects an Authorization: Bearer header. " +
                      "Supports static tokens and dynamic token providers via config-file discovery."
